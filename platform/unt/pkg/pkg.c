...
int main(int argc,char *argv[])
{
	...
	for(i=1;i<=4;i++)
	{
...

		{
			...
			{
				immp=j+1;
				result=fmmpValSet("_immp4",j,&immp,0);
				if(result==-1)
					return -1;
				lmmp=(j+1)*1000;
				result=fmmpValSet("_lmmp4",j,&lmmp,0);
				if(result==-1)
					return -1;
				dmmp=(j+1)*1.1;
				result=fmmpValSet("_dmmp4",j,&dmmp,0);
				if(result==-1)
					return -1;
				sprintf(pmmp,"p%d",j+1);
				result=fmmpValSet("_pmmp4",j,pmmp,0);
				if(result==-1)
					return -1;
			}
		}

		result=fpkgEnc("001",trncode,&fmldata,&tmpdata,&size);
		if(result==-1)
		{
		...
		result=fpkgDec("001",trncode,&fmldata,&tmpdata,size);
		if(result==-1)
		{
		...
		/*
		{
			...
			for(j=0;j<*count;j++)
			{
				result=fmmpRefGet("_immp4",j,&immp,0);
				if(result==-1)
					return -1;
				printf("_immp4_%d:[%d]\n",j,*immp);
				result=fmmpRefGet("_lmmp4",j,&lmmp,0);
				if(result==-1)
					return -1;
				printf("_lmmp4_%d:[%ld]\n",j,*lmmp);
				result=fmmpRefGet("_dmmp4",j,&dmmp,0);
				if(result==-1)
					return -1;
				printf("_dmmp4_%d:[%.2f]\n",j,*dmmp);
				result=fmmpRefGet("_pmmp4",j,&pmmp,0);
				if(result==-1)
					return -1;
				printf("_pmmp4_%d:[%s]\n",j,pmmp);
			}
